...
class TwoFactorAuthTest extends TestCase
{
    ...
    public function enable_2fa_from_profile_page()
    {
        ...
        Authy::shouldReceive('sendTwoFactorVerificationToken')->andReturn(true);
    ...
    /** @test */
    public function cannot_enable_2fa_if_its_already_enabled_for_same_phone_number_and_country_code()
    {
        $this->setSettings(['2fa.enabled' => true]);

        /** @var User $user1 */
        $user1 = UserFactory::user()->create();
        $user2 = UserFactory::user()->twoFactor('1', '123')->create();
        ...
        $this->assertSessionHasError(trans('auth.2fa.phone_in_use'));

        $user1->refresh();

        $this->assertNull($user1->two_factor_country_code);
        $this->assertNull($user1->two_factor_phone);
    }
